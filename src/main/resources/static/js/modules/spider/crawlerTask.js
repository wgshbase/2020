    ...
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '操作', name: 'id', width: 80, formatter: function(value, options, row){
				...
				'<button onclick="vm.delTask(\''+value+'\')" class="btn btn-info">查看</button>';
			}}	
...
var vm = new Vue({
	...
	methods: {
		...
		delTask: function (taskId) {
            ...
            var ret = $("#jqGrid").jqGrid('getRowData',taskId);
            alert("抓取任务中断, 抓取数量是: "+ret.pageCount+"");
			//alert("----" + this);
			...
			//alert("当前任务已经结束,共完成了" + vm.pageCount + "条数据的采集");
		},		
		testTask:function () {        
