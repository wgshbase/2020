...
public class ImageUtils2 {
   ...
   public static String cutImage(String sourcePath , String targetPath , int x , int y , int width , int height) throws IOException{  
    ...
    public static void main(String[] args) throws Exception {  
        ...
        /*try {  
        	new File("D://111.gif");
            zoom("D://111.gif", "D://112.gif", 900, 600);
...
//              ImageCutterUtil.cutImage(file, destFile, x, y, width, height);  
		...
		int status = gd.read(new FileInputStream(new File("D:/111.gif")));
		if (status != GifDecoder.STATUS_OK) {
			return;
		}
		//
		...
		ge.start(new FileOutputStream(new File("D:/114.gif")));
		ge.setRepeat(0);
		...
		for (int i = 0; i < gd.getFrameCount(); i++) {
			...
			//
			BufferedImage rescaled = Scalr.resize(frame, Mode.FIT_EXACT, newWidth, newHeight);
			//
