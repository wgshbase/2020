...
public class TextRank
{
    ...
    private static  int nKeyword=15;         //number of keywords to extract,default 5
    ...
    public static List<String> getKeyword(String title, String content)
    {
        ...
        List<Map.Entry<String, Float>> entryList;
        entryList = new ArrayList<Map.Entry<String, Float>>(score.entrySet());
		try {
        ...
        for (int i = 0; i < nKeyword; ++i){
        	...
        	//unmergedList.add(entryList.get(i).getKey());
                ...
                // 排除停用词范围内的名词
                if(!stopWordSet.contains(keyword) && keyword.length() > 1 && (keyword.matches("[a-zA-Z0-9]*") || keyword.matches("[\\u4e00-\\u9fa5]*[1-9]\\d*") || keyword.matches("[\\u4e00-\\u9fa5]*"))) {
    ...
    public static Map<String,Float> getWordScore(String title, String content)
     {
     	 ...
     	 //segment text into words
    	   List<Term> termList =HanLP.newSegment().seg(title +"。"+ content);
//         List<Term> termList = HanLP.segment(title + content);
//         List<Term> termList = parseBaiduAPI2Terms(title + content);
         

         //for(Term term : termList) {
        	 //if(term.nature.toString().equals("n"))
            // System.out.print(term.word + "/" + term.nature + "/" + term.getFrequency() + " ");
         //}
         ...
         for (Term t : termList)
         {
             ...
             {
                 // 关键词仅仅保留名词 ---- 暂时的方案
                 if(t.nature.toString().startsWith("n") && !t.nature.toString().startsWith("nr")) {
