 ...
 * @author wangdw
 */
public class SubjectModelPageProcessor implements IConfigAblePageProcessor {
	private static final Logger LOG = LogManager.getLogger(SubjectModelPageProcessor.class);
	...
	public SubjectModelPageProcessor(Site site, SpiderConfig config) {
		this.site = site;
		this.config = config;
	...
	public void process(Page page) {
		...
		if (requestType == null || Constants.REQUEST_SEARCH.equals(requestType)) { //搜索页面
			...
			for (String searchText : config.getIndexs()) {
				...
				String url = CrawlerCommonUtils.replaceUrlParam(page.getUrl().get(), "q", searchText_Ui);
				for (int i = 1; i <= 3; i++) {
