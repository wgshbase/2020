...
public class DbBuilder {
	...
	public static DataSource getDataSource() {
		if (DbBuilder.dataSource == null) {
			try {
				...
				dbcpDataSource.setRemoveAbandoned(Boolean.parseBoolean(config.getProperty("dbcp.removeAbandoned")));
				dbcpDataSource.setRemoveAbandonedTimeout(Integer.parseInt(config.getProperty("dbcp.removeAbandonedTimeout")));
				//System.out.println(Boolean.parseBoolean(config.getProperty("dbcp.removeAbandoned")) + "<------------->" + Integer.parseInt(config.getProperty("dbcp.removeAbandonedTimeout")));
	...
	public static void main(String[] args) {
		...
		String sql = "INSERT INTO news_aaaa(title) values(?)";
		List list = new ArrayList<>();
		list.add("lier");
		for(int i = 0; i < 10000; i ++) {
			Connection conn = DbBuilder.getConnection();
			DbBuilder.beginTransaction(conn);
			DbBuilder.save(conn,sql, list.toArray());
			DbBuilder.commit(conn);
			//DbBuilder.close(conn);
			try {
				Thread.sleep(50l);
