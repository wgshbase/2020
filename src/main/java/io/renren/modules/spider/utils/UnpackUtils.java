...
public class UnpackUtils {
   ...
   public static Map extract(Map map) {
       ...
       String namedEntity;
       if(map.containsKey("title_tr")) {
           String content = CSVUtils.getContentWithoutHtmlTag((String) map.get("content_tr"));
           if(StringUtils.isEmpty(content)) {
               return map;
           }
           content = content.replaceAll("信用(.*?)(?<=>)", "").replaceAll("<.*?>", "").replaceAll("\\s+", " ").trim();
           String title = (String)map.get("title_tr");
           keywords = CSVUtils.parseList2String(extractor.extractKeywords(content));

           //summary = CSVUtils.parseList2String(extractor.extractSummary(content));
           summary = Sample.extractNewsSummary(content, 3);
           namedEntity = CSVUtils.parseMap2String(Sample.extractEntities(content));
       } else {
           String content = CSVUtils.getContentWithoutHtmlTag((String) map.get("content"));
           if(StringUtils.isEmpty(content)) {
               return map;
           }
